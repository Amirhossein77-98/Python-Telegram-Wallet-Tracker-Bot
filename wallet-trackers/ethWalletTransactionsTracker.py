...
class ETHTracker:
    def __init__(self):
        ...
        self.api_token = config.get("ETH", "token")

    def get_wallet_transactions(self, wallet_address, last_block_number):
        url = 'https://api.etherscan.io/api?module=account&action=txlist&address={}&sort=desc&startblock={}&endblock=99999999'.format(wallet_address, 0 if last_block_number == None else int(last_block_number) + 1)
        headers = {
...
# block_number, timestamp, tx_hash, block_hash, tx_from, tx_to, tx_value, tx_gas_price, tx_gas_limit, tx_gas_used, tx_fee, tx_status = ETHTracker().get_wallet_transactions(wallet_address="0x47A11b9B6EB71F047d797E44E6BA508Eba9F2e16")
...
# print(f'Transaction Fee: {tx_fee} ETH')
# print(f'Transaction Gas Price: {tx_gas_price} Gwei')
